...
def get_session_id():
    response = [None]
    web_id = request_www_westernunion_com(response)
    return web_id
...
def request_www_westernunion_com(req, response):
    ...
    try:
        req = urllib.request.Request("https://www.westernunion.com/wuconnect/rest/api/v1.0/CreateSession?timestamp={}".format(str(int(time.time()*1000)))) #1537800288827 
